...
export const SignIn = () => {
  const [userProps, setUserProps] = useState({ name: "", email: "", password: "" })

  const handleUserProps = (e) => {
    const { name, value } = e.target;
    setUserProps((prev: UserProps) => ({ ...prev, [name]: value }));
  }
    ...
    <div className='min-h-[80vh] w-full flex justify-center items-center px-4 mt-16'>
      ...
      >
        ...
        <p className="text-center text-gray-500 text-sm mt-6">
          Dont have an account? <span className="text-blue-400 cursor-pointer hover:underline">Sign In</span>
        </p>
      </motion.div>
    </div>
  )
